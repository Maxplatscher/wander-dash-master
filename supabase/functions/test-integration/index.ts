...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (integration.system_type === "rest_api" || integration.system_type === "research_source") {
    ...
    } else if (integration.system_type === "email_imap") {
      const config = (integration.config ?? {}) as Record<string, string>;
      ...
      if (!host) {
        result = { success: false, message: "Kein IMAP-Host konfiguriert" };
      } else if (isPrivateOrLoopback(host)) {
        result = {
          success: false,
          message: "Private oder lokale Adressen sind aus Sicherheitsgründen nicht erlaubt",
        };
      } else if (!username || !password) {
        result = { success: false, message: "IMAP-Benutzername oder Passwort fehlt" };
      } else {
        ...
        try {
          ...
          result = {
            success: true,
            message: `IMAP-Anmeldung erfolgreich (${host}:${port}, Ordner ${folder})`,
            latency_ms: Date.now() - start,
          };
        } catch (error) {
          const raw = error instanceof Error ? error.message : "Verbindung fehlgeschlagen";
          const isTimeout = /zeitüberschritten|timeout/i.test(raw);
          result = {
            success: false,
            message: isTimeout
              ? "Zeitüberschreitung – IMAP-Server nicht erreichbar"
